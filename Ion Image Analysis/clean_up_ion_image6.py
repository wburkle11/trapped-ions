...
#   True  -> use tilt_image's rotated image for PSF/RL cleanup
#   False -> use the unrotated manual ROI image for PSF/RL cleanup
#
# Note: tilt_image is still called either way, because we still use it
# to find ion centers / psf_settings.
USE_TILT_ROTATION = False
...
if USE_TILT_ROTATION:
    rotated_image = tilt_result["rotated_image"]
    image_used_for_cleanup_label = "rotated"
else:
    rotated_image = raw_image.copy()
    image_used_for_cleanup_label = "unrotated"

print(f"\nRotation angle reported by tilt_image: {tilt_result['angle_deg']:.6f} degrees")
print(f"USE_TILT_ROTATION = {USE_TILT_ROTATION}")
print(f"Image used for PSF/RL cleanup: {image_used_for_cleanup_label}")

if tilt_result["rotated_path"] is not None and USE_TILT_ROTATION:
    print(f"Saved rotated image to: {tilt_result['rotated_path']}")
elif tilt_result["rotated_path"] is not None and not USE_TILT_ROTATION:
    print("Rotated image was generated by tilt_image but NOT used for cleanup.")
...
ax2.set_title("chosen PSFs", fontsize=18)
ax2.axis("off")
...
for i, s in enumerate(psf_settings):
    ...
    if i < len(fit_results):
        x0 = fit_results[i]["fit"]["x0"]
        y0 = fit_results[i]["fit"]["y0"]
    else:
        x0 = s["x0"]
        y0 = s["y0"]
...
rl_path = input_path.with_name(input_path.stem + f"_manualROI_{image_used_for_cleanup_label}_rl.tif")
...
for ax, img, title in zip(
    ...
    [f"raw image used for cleanup ({image_used_for_cleanup_label}, px)", "cleaned image (px)"]
):
    ax.imshow(
...
for ax, img, title in zip(
    axes4b,
    [rotated_image, global_rl],
    [f"raw image used for cleanup ({image_used_for_cleanup_label}, um)", "cleaned image (um)"]
):
    ax.imshow(
        display_image(img, DISPLAY_GAMMA),
        cmap=DISPLAY_CMAP,
