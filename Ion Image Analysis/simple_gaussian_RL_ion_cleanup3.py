...
# Background handling
# To avoid the image looking artificially cleaned, this version only removes
# a constant camera/background offset before RL. It does NOT subtract a broad
# smoothed background image.
BACKGROUND_PERCENTILE = 0
SUBTRACT_BROAD_BACKGROUND = False
BACKGROUND_SMOOTH_SIGMA = 15     # only used if SUBTRACT_BROAD_BACKGROUND=True
...
PSF_SIGMA_X = 1.2
PSF_SIGMA_Y = 3.0
PSF_SIZE = 31                    # odd number; try 21, 25, or 31

# Richardson-Lucy settings
RL_ITERATIONS = 7            # try 5-12; too high amplifies scatter
EPS = 1e-12

# Final cleanup
# Keep this gentle to avoid removing raw image character.
FINAL_BG_PERCENTILE = 0
FINAL_SOFT_DENOISE = True
FINAL_DENOISE_SIGMA = 0.25
DISPLAY_GAMMA = 1.0

# Plotting only: makes zoomed image look less blocky.
# This does NOT change the saved TIFF data.
PLOT_INTERPOLATION = "bilinear"   # try "bilinear", "bicubic", or "none" 
...
def subtract_background(img, percentile=5, smooth_sigma=20, subtract_broad=False):
    """
    Gentle background handling.

    This only subtracts a constant percentile offset by default.
    That preserves more of the raw image qualities and avoids the
    over-cleaned look caused by subtracting a broad smoothed background.

    Set subtract_broad=True only if there is a large diffuse glow.
    """
...
def final_cleanup(img, bg_percentile=0, soft_denoise=True, denoise_sigma=0.35):
    """
    Gentle final cleanup.

    If bg_percentile <= 0, no final background clipping is applied.
    This keeps more of the raw background/texture while still allowing
    optional very light smoothing to reduce pixelation.
    """
    out = img.astype(float).copy()

    if bg_percentile is not None and bg_percentile > 0:
        bg = np.percentile(out[out > 0], bg_percentile) if np.any(out > 0) else 0
        out -= bg
        out[out < 0] = 0

    if soft_denoise and denoise_sigma > 0:
...
clean_for_roi, broad_bg_full, offset_full = subtract_background(
    ...
    subtract_broad=SUBTRACT_BROAD_BACKGROUND
)
...
if SAVE_OUTPUT:
    out_path = input_path.with_name(input_path.stem + "_simple_gaussian_RL_no_broad_bg_subtraction.tif")
...
plt.title("cleaned image", fontsize=16)
plt.axis("off")
plt.tight_layout()
plt.show()
...
if MAKE_FINAL_ZOOM_PLOT:
    ...
    plt.title("post processed image", fontsize=16)
    plt.axis("off")
    plt.tight_layout()
    plt.show()
