...
This version uses ONLY THE FIRST FRAME of a TIFF stack.

Pipeline:
1. Load full raw image
2. If TIFF stack, use data[0] only
...
PSF_SELECTION_MODE = "all"
MIDDLE_SKIP = 2
PSF_SELECTION_IONS = [1, 2, 3]

USE_PSF_WEIGHT_MASK = False
PSF_MASK_SIGMA_X_SCALE = 0.5
...
def load_tiff_first_frame(path):
    """
    Load TIFF using tifffile.

    If TIFF is a stack, return ONLY the first frame, data[0].
    """
    ...
    if data.ndim == 2:
        print("Single 2D image detected. Using this image.")
        return data.copy(), data

    elif data.ndim == 3:
        print(f"Image stack detected with {data.shape[0]} frames.")
        print("Using ONLY first frame: data[0].")
        first_frame = data[0].copy()
        return first_frame, data
...
def show_stack_diagnostic(path):
    ...
    if data.ndim == 3:
        ...
        axes[0].set_title("First frame used for cleanup")
        axes[0].axis("off")

        axes[1].imshow(display_image(avg_frame, DISPLAY_GAMMA), cmap="inferno", interpolation="none")
        axes[1].set_title("Average over all frames, not used")
        axes[1].axis("off")
...
def make_auto_roi_image(
    ...
    raw, original_data = load_tiff_first_frame(file_path)
    ny, nx = raw.shape
    ...
    if save_roi:
        ...
        roi_path = in_path.with_name(in_path.stem + "_FIRSTFRAME_ionCrystal_autoROI_noRotation.tif")
        imwrite(roi_path, roi.astype(np.float32))
    else:
        roi_path = None

    print("\n================ ION-CRYSTAL AUTO ROI ================")
    print(f"Original first-frame image shape: {raw.shape}")
    print(f"ROI bounds: x=[{x_min}:{x_max}], y=[{y_min}:{y_max}]")
    print(f"ROI shape: {roi.shape}")
    print(f"Detected ion-like peaks: {len(psf_settings)}")
    if roi_path is not None:
        print(f"Saved first-frame ROI image to: {roi_path}")
    ...
    axes[0].set_title("First Frame with Ion-Crystal Auto ROI")
    axes[0].axis("off")
    ...
    axes[1].set_title("Bandpassed First Frame Used for Ion Detection")
    axes[1].axis("off")
    ...
    axes[2].set_title("Final Auto-Cropped First-Frame Ion ROI, No Rotation")
    axes[2].axis("off")
...
if USE_AUTO_ROI:
    ...
    raw_image, _ = load_tiff_first_frame(file_path)
    roi_bounds = None
...
ax1.set_title("First-Frame Auto-ROI Raw Image, No Rotation", fontsize=18)
ax1.axis("off")
plt.tight_layout()
plt.show()
...
ax2.set_title("First-Frame Auto-ROI Image with PSF Crops, No Rotation", fontsize=18)
ax2.axis("off")
...
if USE_AUTO_ROI:
    rl_path = input_path.with_name(input_path.stem + "_FIRSTFRAME_autoROI_noRotation_rl.tif")
else:
    rl_path = input_path.with_name(input_path.stem + "_FIRSTFRAME_noRotation_rl.tif")
...
axes4[0].set_title("first-frame raw image with ROI, no rotation", fontsize=18)
axes4[0].axis("off")

axes4[1].imshow(display_image(global_rl, DISPLAY_GAMMA), cmap="inferno", interpolation="none")
axes4[1].set_title("cleaned first-frame image, no rotation", fontsize=18)
axes4[1].axis("off")
