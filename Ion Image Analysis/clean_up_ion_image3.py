...
# Center the ROI on the ion crystal before rotation/deconvolution.
# This is the important fix: the saved ROI itself is centered, not just the plot.
CENTER_ROI_ON_IONS = True

# If None, the ROI size is computed from the detected ion cluster span + padding.
# You can also force a fixed ROI size, e.g. CENTERED_ROI_WIDTH = 260.
CENTERED_ROI_WIDTH = None
CENTERED_ROI_HEIGHT = None
...
def centered_bounds(cx, cy, width, height, nx, ny):
    """
    Return image-safe bounds for a crop centered at (cx, cy).

    This keeps the ROI centered on the ion crystal as much as possible.
    If the crop would go outside the image, it shifts back inside while
    preserving the requested width/height.
    """
...
def make_auto_roi_image(
    ...
    if center_roi_on_ions:
        # Compute a centered ROI size from the ion-cluster span plus padding.
