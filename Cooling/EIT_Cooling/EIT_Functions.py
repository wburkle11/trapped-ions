...
delpi = np.linspace(-30*1e6, 80*1e6, 4000)* 2*math.pi  #PROBE DETUNING
...
delpi2 = del_d - zeeman
delpi4 = np.linspace(-30*1e6, 80*1e6, 50000)* 2*math.pi  #PROBE DETUNING
...
omega_sig_m = 2*math.pi * 27*1e6      #Rabi of sigma_minus component of driving beam
omega_sig_p = 2*math.pi * 27*1e6      #Rabi of sigma_plus component of driving beam
omega_pi = 2*math.pi * 2*1e6          #Rabi of probe beam
nu = 2*math.pi*2*1e6                  #Trap Freq. we wish to cool
nu2 = np.linspace(0.5*1e6, 6*1e6, 909)* 2*math.pi  #PROBE DETUNING
...
def ss_master_equation_sol(delpi, del_d, omega_sig_m, omega_sig_p, omega_pi, zeeman):
    ...
    #MASTER EQ TREATMENT DEFINITIONS
    # Define the basis states
    # Assuming a 4-level system: |0>, |+>, |->, |e>
    N = 4
...
pop_master = ss_master_equation_sol(delpi, del_d, omega_sig_m, omega_sig_p, omega_pi, zeeman)
...
#nbar_kim_mas = compute_nbar_ratio(CAR, RED, BLUE)
...
#nbar_kim_smooth2 = gaussian_filter(nbar_kim_mas, sigma=8)  # Adjust sigma for smoothing strength
...
maskI = (delpi4 >= delpi_lower_threshold2) & (delpi4 <= delpi_upper_threshold2)
filtered_delpi = delpi[mask]
filtered_delpi_2 = delpi4[maskI]
filty2_kim = nbar_kim_smooth[mask]
#filty3_kim = nbar_kim_smooth2[maskI]
...
label2 = r'$\delta_{z}$' + f' = {zeeman / (2*math.pi*1e6):.2f} MHz'
label3 = r'$\Omega_{\sigma_{+}} \equiv \Omega_{\sigma_{-}}$' + f' = {omega_sig_p / (2*math.pi*1e6):.2f} MHz'
...
# Add a single axis
ax2 = fig2.add_subplot(111)  # 111 means 1 row, 1 column, 1st subplot
label1 = r'$\Delta_{d}$' + f' = {del_d / (2*math.pi*1e6):.2f} MHz'
label2 = r'$\delta_{z}$' + f' = {zeeman / (2*math.pi*1e6):.2f} MHz'
label3 = r'$\Omega_{\sigma_{+}} \equiv \Omega_{\sigma_{-}}$' + f' = {omega_sig_p / (2*math.pi*1e6):.2f} MHz'
label4 = r'$\Omega_{\pi}$' + f' = {omega_pi / (2*math.pi*1e6):.2f} MHz'
combo_label1 = f'{label1} \n {label2} \n {label3} \n {label4}'
# Plot on the first subplot (ax1)
ax2.plot(delpi / (2*math.pi), pop_master, label=combo_label1)
ax2.set_title('FANO-Profile (Double-EIT) - Master Equation')
ax2.set_xlabel(r'$\Delta_{\pi} / 2\pi$ (Hz)', fontsize = 12)
ax2.set_ylabel('Excited State Population')
liner1 = ax1.axvline(x=delsig_m / (2*math.pi), color='black', linestyle='--', label=r'$\Delta_{\pi} \equiv \Delta = \Delta_{d} - \delta_{B}$' + f' = {delsig_m / (2*math.pi*1e6):.2f} MHz')
liner2 = ax1.axvline(x=delsig_p / (2*math.pi), color='black', linestyle='--', label=r'$\Delta_{\pi} \equiv \Delta = \Delta_{d} + \delta_{B}$' + f' = {delsig_p / (2*math.pi*1e6):.2f} MHz')
custom_line = Line2D([0], [0], color='none')  # Line with no color (invisible line)
ax2.legend([custom_line, liner1, liner2], [combo_label1, r'$\Delta_{\pi} \equiv \Delta_{-} = \Delta_{d} - \delta_{B}$' + f' = {delsig_m / (2*math.pi*1e6):.2f} MHz', r'$\Delta_{\pi} \equiv \Delta_{+} = \Delta_{d} + \delta_{B}$' + f' = {delsig_p / (2*math.pi*1e6):.2f} MHz' ], loc='best', fontsize=12)
...
# Add a single axis
ax3 = fig3.add_subplot(111)  # 111 means 1 row, 1 column, 1st subplot

# Plot on the first subplot (ax1)
ax3.plot(zoomed_delpi / (2*math.pi), zoomed_kim_result)
ax3.set_title(r'FANO-Profile $(\Delta_{\pi} \equiv \Delta_{-} = \Delta_{d} - \delta_{z})$')
...
# Add a single axis
ax5 = fig4.add_subplot(111)  # 111 means 1 row, 1 column, 1st subplot
...
for i in range(1, len(nu2)):
    ...
    if (nbar_kim_2[i-1] < 0.1 and nbar_kim_2[i] > 0.1) or (nbar_kim_2[i-1] > 0.1 and nbar_kim_2[i] < 0.1):
        # Linear interpolation to find the exact x where y=0.1
        x1, x2 = nu2[i-1], nu2[i]
        y1, y2 = nbar_kim_2[i-1], nbar_kim_2[i]
...
# Print the intersection points in MHz
for x in intersection_x_values:
    print(f"Intersection at nu = {x / (2 * math.pi * 1e6):.2f} MHz")
...
if len(intersection_x_values) > 1:
    for i in range(1, len(intersection_x_values)):
        ...
        #print(f"Difference between intersection {i} and {i+1}: {diff / (2 * math.pi * 1e6):.2f} MHz")

custom_label = (r' Bandwidth $(\bar{n} \leq 0.1)$' f' = {diff/(2*math.pi*1e6):.2f} MHz \n'
