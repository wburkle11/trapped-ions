...
Omega_s_minus = 2 * math.pi * 25 * 1e6  # Rabi frequency for sigma- transition
Omega_r = 2 * math.pi * 1.5 * 1e6  # Rabi frequency for r transition
Omega_s_plus = 2 * math.pi * 25 * 1e6  # Rabi frequency for sigma+ transition
Delta_d = 2 * math.pi * 45 * 1e6  # DRIVE DETUNING
delta_B = 2 * math.pi * 5 * 1e6  # Zeeman Splitting
delsig_m = Delta_d - delta_B
delsig_p = Delta_d + delta_B
nu = 2*math.pi*2.5*1e6             #Trap Freq. we wish to cool
...
ax1.set_xlabel('Probe Detuning $\\Delta_{\pi}$ (MHz)')
...
ax1.set_title('Population in $|e\\rangle$ vs Probe Detuning $\\Delta_{\pi}$')
