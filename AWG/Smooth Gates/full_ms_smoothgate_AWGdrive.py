...
def program_smooth_gate_awg(
    ...
    amp_max_frac : float
        max fractional DDS amplitude [0, 1]
    f_red_ref_hz : float
    ...
    if trigger_mode == "ext":
        # Replace EXT0 with the correct external trigger source for your setup if needed
        dds.trg_src(spcm.SPCM_DDS_TRG_SRC_EXT0)
    ...
    # Initial point: zero amplitude at max detuning
    ...
    for t in tvals_g:
        ...
        blue.amp((100.0 * amp_frac) * units.percent)
        red.amp((100.0 * amp_frac) * units.percent)
        dds.exec_at_trg()
    ...
    # Enable trigger, but do not force-trigger when using ARTIQ
